...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            ...
            <p class="mt-1 text-gray-600">Manage administrator accounts and their access</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <form method="GET" class="flex flex-wrap gap-4 items-end">
            ...
            <div class="flex-1 min-w-64">
                <label for="search" class="block text-sm font-medium text-gray-700 mb-1">Search Admins</label>
            ...
            <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-2">
                        {{-- The row checkboxes live in the table and associate with this form
                             via form="bulk-form" — the rows already contain their own action
                             forms, and forms cannot nest. --}}
                        <form method="POST" action="{{ route('admin.admin-users.bulk-status') }}" id="bulk-form" class="flex items-center space-x-2">
                            @csrf
                            @method('PATCH')
                            ...
                            <button type="submit" class="bg-gray-600 text-white px-3 py-2 rounded-md text-sm hover:bg-gray-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" disabled id="bulk-submit">
                                Apply
                            </button>
                        </form>
                    </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <input type="checkbox" name="user_ids[]" form="bulk-form"
                                       class="user-checkbox h-4 w-4 text-[#00BDE0] focus:ring-[#00BDE0] border-gray-300 rounded"
                            ...
                            <td class="px-6 py-4">
                                @forelse($user->adminRoles as $assignedRole)
                                    <span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium mr-1 {{ $assignedRole->is_super_admin ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-700' }}">
                                        {{ $assignedRole->name }}
                                    </span>
                                @empty
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-2">
                                    ...
                                    @if($user->id !== auth()->id())
                                        <form method="POST" action="{{ route('admin.admin-users.toggle-status', $user) }}" class="inline">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit"
                                        ...
                                        <form method="POST" action="{{ route('admin.admin-users.destroy', $user) }}"
                                              class="inline" onsubmit="return confirm('Are you sure you want to delete this admin?')">
                                            @csrf
                                            @method('DELETE')
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Nothing to wire up on an empty list.
    ...
    function updateSelectAll() {
        ...
        selectAll.checked = userCheckboxes.length > 0 && checkedBoxes.length === userCheckboxes.length;
        selectAll.indeterminate = checkedBoxes.length > 0 && checkedBoxes.length < userCheckboxes.length;
    }

    function updateBulkActions() {
        bulkSubmit.disabled = document.querySelectorAll('.user-checkbox:checked:not([disabled])').length === 0;
    }
    ...
    bulkForm.addEventListener('submit', function(e) {
        ...
        if (!confirm(`Are you sure you want to ${action} ${checkedBoxes.length} admin(s)?`)) {
            e.preventDefault();
        }
    });
