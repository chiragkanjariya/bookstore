                ...
                <div class="border-b border-gray-200 mb-6">
                    <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                        ...
                        <button type="button" id="tab-btn-bulk-shipping" onclick="switchBookTab('bulk-shipping')"
                            ...
                            @if(count($book->bulkStateShippingPrices) > 0)
                                <span class="ml-2 bg-[#00BDE0] text-white text-xs px-2 py-0.5 rounded-full">{{ count($book->bulkStateShippingPrices) }}</span>
                            @endif
                        </button>
                    </nav>
                </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div>
                            ...
                            <select name="language" id="language" required
                                ...
                                <option value="Gujarati" {{ old('language', $book->language) === 'Gujarati' ? 'selected' : '' }}>Gujarati</option>
                            </select>
                            @error('language')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
            ...
            <div id="tab-content-shipping" class="book-tab-content hidden">
                ...
                <div id="tab-content-bulk-shipping" class="book-tab-content hidden">
                    <div class="bg-gray-50 rounded-lg p-6 mb-6 border border-gray-200">
                        ...
                        <p class="text-sm text-gray-600 mb-4">
                            Configure state-wise shipping prices applied when this book's order qualifies as a <span class="font-semibold">bulk purchase</span>. If a state field is left blank, the <strong>Default Bulk Shipping Price</strong> below will be used during bulk order checkout.
                        </p>
                        ...
                        <div class="bg-white p-4 rounded-lg border border-gray-200 shadow-sm mb-6 max-w-sm">
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="bulk_shipping_price" id="bulk_shipping_price"
                                       value="{{ old('bulk_shipping_price', $book->bulk_shipping_price) }}" min="0" step="0.01"
                                    ...
                                    <div class="bg-white p-3.5 rounded-lg border border-gray-200 shadow-sm flex flex-col justify-between">
                                        ...
                                        <div class="relative">
                                            ...
                                            <input type="number"
                                                   ...
                                                   value="{{ old('bulk_state_shipping_prices.'.$state->id, $bulkVal) }}"
                                                   min="0" step="0.01"
                                                   placeholder="Default (₹{{ number_format($book->bulk_shipping_price, 2) }})"
                                                   class="w-full pl-7 pr-3 py-1.5 border border-gray-300 rounded-md focus:outline-none focus:ring-[#00BDE0] focus:border-[#00BDE0] text-sm">
                                        </div>
                                    </div>
                                @endforeach
                        ...
                        @else
                            <div class="bg-yellow-50 border border-yellow-200 rounded-md p-4 text-center">
                                <p class="text-sm text-yellow-800">No states found in the system database. Default bulk shipping price will apply to all bulk orders.</p>
                            </div>
                        @endif
                    </div>
                </div>
